...
export default function AddReadingPassageToEsp() {
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    try {
      const res = await espQuestionsAPI.createReadingPassage({
        ...form,
        esp_skill: skillId, // ← esp_skill مش ielts_skill
      });
      navigate(
        `/dashboard/esp/skills/${skillId}/add/reading/passage/${res.passage.id}/questions`
      );
    } catch (err) {
      setErrors(err.response?.data || { esp: "حدث خطأ، حاول مرة أخرى" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Link
          to={`/dashboard/esp/skills/${skillId}`}
