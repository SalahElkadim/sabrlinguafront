...
export default function EditLessonPack() {
  ...
  const fetchLessonPackData = async () => {
    try {
      setFetching(true);
      ...
      setFormData({
        ...
        exam_time_limit: data.exam_time_limit || 30,
        exam_passing_score: data.exam_passing_score || 70,
        order: data.order || 0,
        is_active: data.is_active ?? true,
      });
    } catch (error) {
      console.error("Error fetching lesson pack:", error);
      toast.error("فشل في تحميل بيانات Lesson Pack");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await ieltsLessonPacksAPI.update(packId, submitData);
      toast.success(response.message || "تم تحديث Lesson Pack بنجاح");
      navigate(`/dashboard/ielts/lesson-packs/${packId}`);
    } catch (error) {
      ...
      if (error.response?.data) {
        ...
        if (typeof serverErrors === "object" && !serverErrors.error) {
          ...
          toast.error(serverErrors.error || "فشل في تحديث Lesson Pack");
        }
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">تحديث بيانات مجموعة الدروس</p>
        </div>
        <Link
          to={`/dashboard/ielts/lesson-packs/${packId}`}
