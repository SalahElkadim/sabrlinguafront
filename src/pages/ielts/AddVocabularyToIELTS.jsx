...
export default function AddVocabularyToIELTS() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (filledOptions.length < 2) {
      setErrors({ options: "At least two options must be added" });
      return;
    }
    setLoading(true);
    setErrors({});
    try {
      await ieltsQuestionsAPI.createVocabulary({
        ...form,
        options,
        correct_answer: options[correctIndex],
        ielts_skill: skillId,
      });
      navigate(`/dashboard/ielts/skills/${skillId}`);
    } catch (err) {
      setErrors(
