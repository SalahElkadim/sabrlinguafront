// src/pages/payment/IeltsPaymentPage.jsx
import { useEffect, useState, useRef } from "react";
import { useSearchParams } from "react-router-dom";
import axios from "axios";

export default function IeltsPaymentPage() {
  const [searchParams] = useSearchParams();
  const token = searchParams.get("token");

  const [ielts, setIelts] = useState("loading"); // loading | select | paying | error
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_URL}/ielts/subscription/plans/`, {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const callbackUrl =
        `${window.location.origin}/ielts/payment/callback` +
      ...
      window.Moyasar.init({
        ...
        currency: "SAR",
        description: `اشتراك IELTS - ${selectedPlan.name}`,
  ...
  }, [ielts, moyasarReady, selectedPlan, token]);
        ...
        <div className="pay-card">
          ...
          {ielts === "select" && (
            <div style={{ animation: "fadeIn 0.4s ease" }}>
              <p className="plans-title">اختر خطة اشتراك IELTS</p>
              <p className="plans-subtitle">
                استمر في حل أسئلة اختبار IELTS بدون قيود مع خطة مناسبة لك
              </p>
              {plans.map((plan, idx) => (
                <div
                  key={plan.id}
